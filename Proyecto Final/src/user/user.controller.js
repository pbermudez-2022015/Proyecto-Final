...
export const getUser = async (req, res) => {
    try {
        let { id } = req.params;
        let user = await User.findById(id); // Utilizar User en lugar de Cliente
        if (!user) {
            return res.status(404).send({ message: 'User not found' }); // Cambiar 'Cliente not found' a 'User not found'
        }
        return res.send({ user });
    } catch (err) {
        console.error(err);
